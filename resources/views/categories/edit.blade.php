...
@section('content')
    ...
    <form class="form-floating text-center m-auto text-capitalize w-50 mt-2"
                ...
                <div class="">
                    {{-- <select name="hs_code" id="floatingSelectGrid" class="js-example-basic-single w-75 fs-2">
                        ...
                        @foreach (App\Models\HsCode::whereNull('parent_id')->get() as $code)
                            <option class="text-capitalize" value="{{ $category->hs_code }}" @if ($category->hs_code == $category->hs_code)
                                selected
                        @endif>{{ $code->code }} ||
                        {{ $code->details }}</option>
                        @if ($code->children())
                    ...
                    <select name="hs_code" id="hs_code" class="js-example-basic-single w-75 fs-2">
                        @foreach (App\Models\HsCode::whereNull('parent_id')->get() as $c)
                            <option class="text-capitalize" value="{{ $c->id }}" @if ($category->hs_code == $c->id)
                                selected
                        @endif
                        @if ($c->id == $category->hs_code)
                            disabled
                        @endif>{{ $c->code }} ||
                        {{ $c->details }}</option>
                        @if ($c->children())
                            @foreach ($c->children as $lv1)
                                <option class="text-capitalize" value="{{ $lv1->id }}" @if ($category->hs_code == $lv1->id) selected @endif
                                    @if ($lv1->id == $category->hs_code) disabled @endif>
                                    --{{ $lv1->code }} || {{ $lv1->details }}</option>
                                @foreach ($lv1->children as $lv1Cild)
                                    <option class="text-capitalize" value="{{ $lv1Cild->id }}" @if ($category->hs_code == $lv1Cild->id) selected @endif
                                        @if ($lv1Cild->id == $category->hs_code) disabled @endif>
                                        ---{{ $lv1Cild->code }} || {{ $lv1Cild->details }}</option>
                                        @foreach ($lv1Cild->children as $lv4Cild)
                                        <option class="text-capitalize" value="{{ $lv4Cild->id }}" @if ($category->hs_code == $lv4Cild->id) selected @endif
                                        @if ($lv4Cild->id == $category->hs_code) disabled @endif>
        ...
        <div class="row m-2">
            <div class="col-md m-auto">
                <div class="form-floating">
                    <select name="parent_id" id="parent_id" class="js-example-basic-single w-75 fs-2">
