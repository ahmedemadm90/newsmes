...
@section('content')
                ...
                <select name="categories_id[]" id="floatingSelectGrid" class="js-example-basic-multiple w-100" multiple>
                    @if (isset($project->categories_id))
                        @foreach ($categories as $cat)
                            <option class="text-capitalize" value="{{ $cat->id }}" @if (in_array($cat->id, $project->categories_id))
                                selected
                        @endif>{{ $cat->en_name }} ||
                        {{ $cat->ar_name }}</option>
                        @if ($cat->children())
                            @foreach ($cat->children as $item)
                                <option class="text-capitalize" value="{{ $item->id }}" @if (in_array($item->id, $project->categories_id))
                                    selected
                            @endif> --{{ $item->en_name }}
                            || {{ $item->ar_name }}</option>
                            @if ($item->children())
                                @foreach ($item->children as $item1)
                                    <option class="text-capitalize" value="{{ $item1->id }}" @if (in_array($item1->id, $project->categories_id))
                                        selected
                                @endif>
                                ---{{ $item1->en_name }}
                    ...
                    <div class="iner">
                        <select name="hs_code[]" id="multiple_code" class="js-example-basic-multiple w-75 fs-2" multiple>
                            @foreach (App\Models\HsCode::whereNull('parent_id')->get() as $code)
                                <option class="text-capitalize" value="{{ $code->id }}" @if (in_array($code->id, $project->hs_code))
                                    selected
                            @endif>{{ $code->code }} ||
                            {{ $code->details }}</option>
                            @if ($code->children())
                                @foreach ($code->children as $item)
                                    <option class="text-capitalize" value="{{ $item->id }}" @if (in_array($item->id, $project->hs_code))
                                        selected
                                @endif> --{{ $item->code }}
                                || {{ $item->details }}</option>
                            @endforeach
    ...
    @else
        <div class="col-md">
            <h2 class="text-danger text-decoration-underline">{{ __('Select Code') }}</h2>
            <div class="iner">
                @if (!empty($project->hs_code))
                    <select name="hs_code[]" id="multiple_code" class="js-example-basic-multiple w-100 fs-2" multiple>
                        @foreach (App\Models\HsCode::whereNull('parent_id')->get() as $code)
                            <option class="text-capitalize" value="{{ $code->id }}" @if (in_array($code->id, $project->hs_code)) selected @endif>
                                {{ $code->code }} ||
                                {{ $code->details }}</option>
                            @if ($code->children())
                                @foreach ($code->children as $child)
                                    <option class="text-capitalize" value="{{ $child->id }}" @if (in_array($child->id, $project->hs_code)) selected @endif>
                                        --{{ $child->code }}
                                        || {{ $child->details }}</option>
                                @endforeach
                            @endif
                        @endforeach
                    </select>
                @else
                    <select name="hs_code[]" id="multiple_code" class="js-example-basic-multiple w-100 fs-2" multiple>
                        @foreach (App\Models\HsCode::whereNull('parent_id')->get() as $code)
                            ...
                            @if ($code->children())
                                @foreach ($code->children as $child)
                                    <option class="text-capitalize" value="{{ $child->id }}"> --{{ $child->code }}
                                        || {{ $child->details }}</option>
                                @endforeach
                            @endif
                        @endforeach
