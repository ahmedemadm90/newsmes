    ...
    <form class="form-floating text-center m-auto text-capitalize w-100 mt-2">
        ...
        <div class="row  m-2">
            <div class="col-md-6 m-auto mb-2">
                <div class="form-floating">
                    <input type="text" class="form-control" id="floatingInput" placeholder="name"
                        value="{{ $machine->vendor->name }}" readonly>
        ...
        <div class="row">
            @foreach ($machine->categories_id as $id)
                <div class="col-md-3 m-2 m-auto">
                    <span class="badge bg-success">{{ $machine->categories($id)->en_name }} ||
                        {{ $machine->categories($id)->ar_name }}</span>
                </div>
            @endforeach
        </div>
        <hr>
                    ...
                    <div class="row">
                        @foreach ($machine->gallery as $img)
                            <div class="carousel-item {{ $loop->first ? 'active' : '' }}">
                                <img src='{{ asset("public/media/machines/images/$img") }}' style="height: 240px"
                                    class="w-75">
                            </div>
                        @endforeach
                    </div>
                    <button class="btn btn-secondary m-3" type="button" data-bs-target="#carouselExampleControlsNoTouching"
                        data-bs-slide="prev">{{ __('Previous') }}</button>
                    <button class="btn btn-primary m-3" type="button" data-bs-target="#carouselExampleControlsNoTouching"
